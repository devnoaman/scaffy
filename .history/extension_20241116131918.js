...
function activate(context) {
 ...
 // Create the project folder
    ...
    // Create a terminal and run the flutter pub get command
	 ...
	 exec(command, { cwd: appsPath }, (err, stdout, stderr) => {
	   if (err) {
	   ...
	   const appUri = vscode.Uri.file(appsPath);
	   vscode.commands.executeCommand('vscode.openFolder', appsPath, true);
	 });
    ...
    const terminal = vscode.window.createTerminal({
	  ...
	  terminal.sendText("activate melos globaly packages\n"); 
	  terminal.sendText('dart pub global activate melos'); // Run the flutter pub get command

	  terminal.sendText("Getting packages\n"); 
	  terminal.sendText('dart pub add melos --dev'); 
	  ...
	  terminal.sendText("echo 'Bootstrap application' "); 

	  terminal.sendText('melos bootstrap'); // Run the flutter pub get command
	  terminal.show(); // Show the terminal
	 

	});

	 // Ask the user whether to open the newly created Flutter app
	 vscode.window.showInformationMessage(`Do you want to open the newly created Flutter app?`, 'Open', 'Cancel')
	 .then(selection => {
	   if (selection === 'Open') {
		 // Open the newly created Flutter app in a new VS Code window
		 const appUri = vscode.Uri.file(basePath);
		 vscode.commands.executeCommand('vscode.openFolder', appUri, true);
	   } else {
		 vscode.window.showInformationMessage('Flutter app creation was successful. You can open it later.');
	   }
	 });
